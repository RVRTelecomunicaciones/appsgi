(function(d) {
    d.addEventListener('DOMContentLoaded', () => {
        ...
        const botonResumen = d.getElementById('lnkResumen');
        ...
        const iconFechaCreacion = d.getElementById('iconFechaCreacion');
        ...
        const filters = (link) => {
            ...
            if (iconFechaCreacion.classList.contains('fa-sort-amount-desc')) {
                fd.append('order', 'fecha_creacion');
                fd.append('order_type', 'DESC');
            } else if (iconFechaCreacion.classList.contains('fa-sort-amount-asc')){
                fd.append('order', 'fecha_creacion');
                fd.append('order_type', 'ASC');
            }
        ...
        const listCoordinaciones = (fd) => {
            tableBody.innerHTML = '<tr><td colspan="14" align="center"><h1><i class="fa fa-spinner fa-pulse fa-fw"></i></h1></td></tr>';

            ajax('post', './searchGeneradas', fd)
                .then((response) => {
                    ...
                    if (records != '') {
		        		...
		        		tableBody.innerHTML = 	`<tr>
		        									<td colspan="14" align="center">NO HAY REGISTROS</td>
					        					</tr>`;
                    }
		        	...
		        	if (inputCorrelativo.value.trim() == '' && $('#selectEstado').val() == '' && inputSolicitante.value.trim() == '' && inputCliente.value.trim() == '' && $('#selectServicioTipo').val() == '' && inputDireccion.value.trim() == '' && selectPerito.value == '' && selectDigitador.value == '' && selectControlCalidad.value == '' && selectCoordinador.value == '' && (inputFechaDesde.value == '' || inputFechaHasta.value == '') ) //&& selectRiesgo.value == '' && selectFechaTipo.value == '')
        ...
        botonImprimir.addEventListener('click', e => {
            ...
            form.setAttribute('action', `generadas/imprimir`);
        ...
        botonExportXls.addEventListener('click', e => {
            ...
            let form = d.createElement('form');
            form.setAttribute('method', 'post');
            form.setAttribute('action', `generadas/exportarExcel`);
            form.setAttribute('target', '_blank');
            let order = '';
            let order_type = '';
        ...
        botonResumen.addEventListener('click', e => {
            ...
            let form = d.createElement('form');
            form.setAttribute('method', 'post');
            form.setAttribute('action', `generadas/resumen`);
            form.setAttribute('target', '_blank');
            let order = '';
            let order_type = '';
            ...
            th.addEventListener('click', e => {
                ...
                switch (parseInt(indice)) {
                    ...
                    case 10:
                        ...
                        if (iconFechaCreacion.classList.contains('fa-sort-amount-desc')) {
                            iconFechaCreacion.classList.remove('fa', 'fa-sort-amount-desc');
                            iconFechaCreacion.classList.add('fa', 'fa-sort-amount-asc');
                            //fd.append('order_type', 'ASC');
                        } else {
                            iconFechaCreacion.classList.remove('fa', 'fa-sort-amount-asc');
                            iconFechaCreacion.classList.add('fa', 'fa-sort-amount-desc');
                            //fd.append('order_type', 'DESC');
                        }
                        break;
                }
