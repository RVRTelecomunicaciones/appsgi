(function(d) {
    d.addEventListener('DOMContentLoaded', () => {
        ...
        const inputSolicitanteNombre = d.querySelector('#mdlSolicitante #inputNombreCompleto');

        const inputSearchSolicitanteNombre = d.querySelector('#mdlSolicitante #inputSearchSolicitanteNombre');
                ...
                .then((respuesta) => {
                    ...
                    if (registros != false) {
                        ...
                        const filasCombo = registrosCombo.map((item, index) => {
                            if (index == 0)
                                return `<option value=""></option><option value="${item.solicitante_id}">${item.solicitante_nombre.toUpperCase()}</option>`
                            else
                                return `<option value="${item.solicitante_id}">${item.solicitante_nombre.toUpperCase()}</option>`
                        }).join("");
                        ...
                        if (inputSearchSolicitanteNombre.value == "")
                        ...
                        link_editar.forEach(link => {
                            link.addEventListener('click', e => {
                                ...
                                inputSolicitanteNombre.value = registros[indice].solicitante_nombre;
                            });
                        });
                    }
                })
        ...
        const filters = (quantity, link) => {
            ...
            fd.append('solicitante_nombre', inputSearchSolicitanteNombre.value)
        ...
        const validarData = (link = false) => {
            ...
            {
                if (link === false)
                    $('#tbl_tasacion_solicitante tbody').html('<tr><td colspan="3"><div style="font-size: 1.5rem">NO SE ENCONTRARON REGISTROS</div></td></tr>');
                else (link > 0)
                    $('#tbl_tasacion_solicitante tbody').html('<tr><td colspan="3"><h1><i class="fa fa-spinner fa-pulse fa-fw"></i></h1></td></tr>');
            }
        }

        inputSearchSolicitanteNombre.addEventListener('keyup', e => {
            if(e.keyCode == 13)
                validarData(1);
        });
        ...
        const crudTasCliente = () => {
            ...
            fd.append('solicitante_nombre', inputSolicitanteNombre.value.toUpperCase())
        ...
        botonGuardar.addEventListener('click', e => {
            e.preventDefault();
            if (inputSolicitanteNombre.value == "" || inputSolicitanteNombre.value.length < 10) {
                swal({
        ...
        botonCancelar.addEventListener('click', e => {
            ...
            inputSolicitanteNombre.value = "";
        });

        botonClose.addEventListener('click', e => {
            inputSearchSolicitanteNombre.value = "";
            botonCancelar.click();
        });
    })
})(document);