(function(d) {
    d.addEventListener('DOMContentLoaded', () => {
        ...
        const inputClienteId = d.querySelector('#mdlCliente #inputId');
        const inputClienteNombre = d.querySelector('#mdlCliente #inputNombreCompleto');

        const inputSearchClienteNombre = d.querySelector('#mdlCliente #inputSearchClienteNombre');
                ...
                .then((respuesta) => {
                    ...
                    if (registros != false) {
                        ...
                        const filasCombo = registrosCombo.map((item, index) => {
                            if (index == 0)
                                return `<option value=""></option><option value="${item.cliente_id}">${item.cliente_nombre.toUpperCase()}</option>`
                            else
                                return `<option value="${item.cliente_id}">${item.cliente_nombre.toUpperCase()}</option>`
                        }).join("");
                        ...
                        if (inputSearchClienteNombre.value == "")
                        ...
                        link_editar.forEach(link => {
                            link.addEventListener('click', e => {
                                ...
                                inputClienteId.value = registros[indice].cliente_id;
                                inputClienteNombre.value = registros[indice].cliente_nombre;
                            });
                        });
                    }
                })
        ...
        const filters = (quantity, link) => {
            ...
            fd.append('cliente_nombre', inputSearchClienteNombre.value)
        ...
        const validarData = (link = false) => {
            ...
            {
                if (link === false)
                    $('#tbl_tasacion_cliente tbody').html('<tr><td colspan="3"><div style="font-size: 1.5rem">NO SE ENCONTRARON REGISTROS</div></td></tr>');
                else (link > 0)
                    $('#tbl_tasacion_cliente tbody').html('<tr><td colspan="3"><h1><i class="fa fa-spinner fa-pulse fa-fw"></i></h1></td></tr>');
            }
        }

        inputSearchClienteNombre.addEventListener('keyup', e => {
            if(e.keyCode == 13)
                validarData(1);
        });

        const crudTasCliente = () => {
            const apiRestMantenimiento = inputClienteId.value == 0 ? '../tascliente/insertCliente' : '../tascliente/updateCliente';

            const fd = new FormData();
            fd.append('cliente_id', inputClienteId.value)
            fd.append('cliente_nombre', inputClienteNombre.value.toUpperCase())
                ...
                .then((respuesta) => {
                    if (respuesta.success) {
                        swal({
                            icon: 'success',
                            title: 'Guardado',
                            text: inputClienteId.value == 0 ? 'Se ha guardado correctamente ...' : 'Se ha actualizado correctamente ...',
                            timer: 3000,
                            buttons: false
                        });
                        ...
                        swal({
                            icon: 'danger',
                            title: 'Error',
                            text: inputClienteId.value == 0 ? 'No se pudo registrar ... Por favor comunicarse con el area de sistemas ...!' : 'No se pudo actualizar ... Por favor comunicarse con el area de sistemas ...!',
                            timer: 3000,
                            buttons: false
                        });
                    }
        ...
        botonGuardar.addEventListener('click', e => {
            e.preventDefault();
            if (inputClienteNombre.value == "" || inputClienteNombre.value.length < 10) {
                swal({
        ...
        botonCancelar.addEventListener('click', e => {
            e.preventDefault();
            inputClienteId.value = "0";
            inputClienteNombre.value = "";
        });

        botonClose.addEventListener('click', e => {
            inputSearchClienteNombre.value = "";
            botonCancelar.click();
        });
    })
})(document);