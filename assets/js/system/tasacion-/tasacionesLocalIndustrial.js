(function(d) {
    d.addEventListener("DOMContentLoaded", ()=>{
        ...
        const listTasacionesLocalIndustrial = () => {
            ajax('get', listaTasacionesLocalIndustrial)
                .then((respuesta) => {
                    ...
                    const filas = registros.map((item, indice) => {
                        return `
                            <tr>
                                ...
                                <td>${item.areas_complementarias}</td>
                                <td>
                    ...
                    table.columns([13]).every( function () {
                        var that = this;
                        $('#inputAreasComplementarias').keypress( function (e) {
                            var keycode = (e.keyCode ? e.keyCode : e.which);
                            if (keycode == '13') {
                                if ( that.search() !== this.value ) {
                                    that
