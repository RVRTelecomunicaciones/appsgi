(function(d) {
    d.addEventListener('DOMContentLoaded', () => {
        ...
        const liTitulo = d.getElementById('liTitulo');
        const labelProcesoCodigo = d.getElementById('labelProcesoCodigo');
        const labelCoordinacionCodigo = d.getElementById('labelCoordinacionCodigo');
        const selectDevivarA = d.getElementById('selectDevivarA');
        const inputObservacion = d.getElementById('inputObservacion');
        const buttonProcesoCancelar = d.getElementById('buttonProcesoCancelar');
        const buttonProcesoGuardar = d.getElementById('buttonProcesoGuardar');

        const filters = (link) => {
            const fd = new FormData()
            fd.append('coordinacion_correlativo', inputCorrelativo.value);
            fd.append('coordinacion_estado', '7');
        ...
        const listCoordinaciones = (fd) => {
            ...
            ajax('post', 'search', fd)
                .then((response) => {
                    ...
                    //if (inputCorrelativo.value.trim() == '' && inputSolicitante.value.trim() == '' && inputCliente.value.trim() == '' && $('#selectServicioTipo').val() == '' && inputDireccion.value.trim() == '' && selectPerito.value == '' && selectDigitador.value == '' && selectControlCalidad.value == '' && selectCoordinador.value == '' && selectRiesgo.value == '')
                    ...
                    let linkIniciarProceso = d.querySelectorAll('#lnkInicio');
                    linkIniciarProceso.forEach(link => {
                        link.addEventListener('click', e => {
                            e.preventDefault();

                            let fd = new FormData();
                            fd.append('proceso_coordinacion', link.dataset.coordinacion);
                            fd.append('proceso_enviado_de', '3');
                            fd.append('proceso_observacion', '');
                            fd.append('proceso_estado', '2');

                            ajax('post', '../proceso/insert', fd)
                                .then((response) => {
                                    if (response.success) {
                                        swal({
                                            icon: 'success',
                    ...
                    let linkFinalizarProceso = d.querySelectorAll('#lnkFinalizar');
                    linkFinalizarProceso.forEach(link => {
                        link.addEventListener('click', e => {
                            e.preventDefault();
                            labelProcesoCodigo.innerText = link.dataset.proceso;
                            labelCoordinacionCodigo.innerText = link.dataset.coordinacion;
                            
                            selectDevivarA.options[3].classList.add('hidden');
                            selectDevivarA.options[4].classList.remove('hidden');
                    ...
                    let linkObservaciones = d.querySelectorAll('#lnkObservaciones');
                    linkObservaciones.forEach(link => {
                        link.addEventListener('click', e => {
                            e.preventDefault();
                            listObservaciones(link.dataset.coordinacion, 3);

                            $('#mdl_observaciones').modal({
                                'show': true,
                                'keyboard': false,
                                'backdrop': 'static'
                            });
        ...
        buttonProcesoGuardar.addEventListener('click', e => {
            let fd = new FormData();
            fd.append('proceso_codigo', labelProcesoCodigo.innerText);
            fd.append('proceso_coordinacion', labelCoordinacionCodigo.innerText);
            fd.append('proceso_enviado_a', selectDevivarA.value);
            fd.append('proceso_observacion', inputObservacion.value);
            fd.append('proceso_estado', '3');

            ajax('post', '../proceso/update', fd)
                .then((response) => {
                    if (response.success) {
                        swal({
                            icon: 'success',
                        ...
                        .then(
                            () => listCoordinaciones(filters(1), buttonProcesoCancelar.click())
                        );
                    } else {
                        swal({
                            icon: 'info',
