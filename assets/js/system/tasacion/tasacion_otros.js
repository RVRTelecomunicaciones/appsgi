(function(d) {
    d.addEventListener("DOMContentLoaded", ()=>{
        ...
        const apiRestListar = 'searchOtros';
        ...
        const filters = (link) => {
            ...
            fd.append('otros_coordinacion', inputCoordinacion.value)
            fd.append('otros_tipo', selectTipo.value)
            fd.append('otros_observacion', inputObservacion.value)
            fd.append('otros_fecha_desde', inputFechaDesde.value)
            fd.append('otros_fecha_hasta', inputFechaHasta.value)
        ...
        const listTasacion = (fd) => {
            tableBody.innerHTML = '<tr><td colspan="6" align="center"><h1><i class="fa fa-spinner fa-pulse fa-fw"></i></h1></td></tr>';
            ...
            ajax('post', apiRestListar, fd)
                .then((response) => {
                    ...
                    if (records != false) {
                        const row = records.map((item, index) => {
                            return `<tr>
                                        ...
                                        <td><div align="left">${item.observacion.toUpperCase()}</div></td>
                                        <td>
                                            <div class='btn-group'>
		        	...
		        	if (inputCoordinacion.value.trim() == '' && selectTipo.value == '' && inputObservacion.value.trim() == '' && inputFechaDesde.value.trim() == '' && inputFechaHasta.value.trim() == '')
